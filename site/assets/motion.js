...
(function () {
  ...
  if (backdrop) {
    ...
    const SEARCH_INDEX = [
      // Pages
      { type: 'page', title: 'Início', desc: '/', url: '/', icon: 'home' },
      { type: 'page', title: 'Serviços', desc: '/servicos', url: '/servicos', icon: 'scissors' },
      { type: 'page', title: 'Equipe', desc: '/equipe', url: '/equipe', icon: 'users' },
      { type: 'page', title: 'Galeria', desc: '/galeria', url: '/galeria', icon: 'image' },
      { type: 'page', title: 'Localização', desc: '/localizacao', url: '/localizacao', icon: 'pin' },
      { type: 'page', title: 'Agendar visita', desc: '/agendar', url: '/agendar', icon: 'calendar' },
      { type: 'page', title: 'Blog', desc: '/blog', url: '/blog', icon: 'book' },
      { type: 'page', title: 'Entrar / Criar conta', desc: '/login', url: '/login', icon: 'user' },
      // Services
      { type: 'service', title: 'Corte Clássico', desc: 'R$ 65 · 45 min', url: '/servicos', icon: 'scissors' },
      { type: 'service', title: 'Degradê Premium', desc: 'R$ 75 · 55 min', url: '/servicos', icon: 'scissors' },
      { type: 'service', title: 'Barba Navalha', desc: 'R$ 55 · 40 min', url: '/servicos', icon: 'scissors' },
      { type: 'service', title: 'Combo Completo', desc: 'R$ 110 · 90 min', url: '/servicos', icon: 'scissors' },
      { type: 'service', title: 'Tratamento Capilar', desc: 'R$ 90 · 60 min', url: '/servicos', icon: 'scissors' },
      { type: 'service', title: 'Experiência VIP', desc: 'R$ 180 · 120 min', url: '/servicos', icon: 'star' },
      // Team
      { type: 'team', title: 'Pedro Sigismundo', desc: 'Founder · barbeiro', url: '/equipe', icon: 'user' },
      { type: 'team', title: 'André Patric', desc: 'Co-founder · barbeiro', url: '/equipe', icon: 'user' },
      // Quick actions
  ...
  (function authNav() {
    ...
    // SECURITY: auth.name / auth.email come from user input stored in
    // localStorage. They are NEVER interpolated into innerHTML. The static
    // markup (SVGs, layout) is built via innerHTML; every user-controlled
    // value is written with textContent so it cannot inject markup/scripts.
    const wrap = document.createElement('div');
    wrap.style.cssText = 'position:relative; display:inline-flex;';
    wrap.innerHTML = `
      <button class="icon-btn auth-avatar" aria-label="Conta" aria-expanded="false" aria-haspopup="menu" style="width:auto; padding:0 10px 0 6px; gap:8px;">
        <span data-avatar-initial style="width:28px; height:28px; border-radius:50%; background:linear-gradient(135deg,var(--blue),var(--blue-deep)); color:#fff; display:inline-flex; align-items:center; justify-content:center; font-family:var(--display); font-weight:600; font-size:13px;"></span>
        <span data-avatar-firstname style="font-family:var(--mono); font-size:12px; color:var(--ink-soft);"></span>
      ...
      <div class="auth-menu" role="menu" style="position:absolute; right:0; top:calc(100% + 8px); min-width:200px; background:var(--bg-2); border:1px solid var(--stroke-hot); border-radius:10px; padding:6px; box-shadow:0 16px 40px rgba(0,0,0,0.4); display:none; z-index:120;">
        <div style="padding:12px 14px 14px; border-bottom:1px solid var(--stroke-soft);">
          <div data-auth-name style="font-family:var(--display); font-size:14px; font-weight:500; color:var(--ink);"></div>
          <div data-auth-email style="font-family:var(--mono); font-size:11px; color:var(--ink-mute); margin-top:2px;"></div>
        </div>
        <a href="agendar.html" role="menuitem" style="display:flex; align-items:center; gap:10px; padding:10px 14px; font-family:var(--mono); font-size:13px; color:var(--ink-soft); border-radius:6px; transition: background 180ms;">
        ...
        <button data-logout role="menuitem" style="width:100%; text-align:left; display:flex; align-items:center; gap:10px; padding:10px 14px; font-family:var(--mono); font-size:13px; color:var(--ink-soft); background:transparent; border:0; cursor:pointer; border-radius:6px; transition: background 180ms; margin-top:2px;">
    ...
    // Inject user-controlled values safely (textContent escapes everything).
    wrap.querySelector('[data-avatar-initial]').textContent = initial;
    wrap.querySelector('[data-avatar-firstname]').textContent = firstName.toLowerCase();
    wrap.querySelector('[data-auth-name]').textContent = auth.name || 'Cliente';
    wrap.querySelector('[data-auth-email]').textContent = auth.email;
    ...
    trigger.addEventListener('click', (e) => {
      e.stopPropagation();
      setMenu(menu.style.display !== 'block');
    });
